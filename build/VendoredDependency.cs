...
";
        ...
        if (string.Equals(Path.GetExtension(filePath), ".cs", StringComparison.OrdinalIgnoreCase))
        {
            RewriteFileWithTransform(
                ...
                content =>
                {
                    ...
                    content = FixKnownNullabilityIssues(filePath, content);
                    ...
                    else
                    {
                        ...
                        builder.AppendLine(GenerateWarningDisablePragma(filePath));
                    ...
                    if (filePath.Contains("AssemblyProcessor.cs"))
                    {
                        ...
                        builder.Replace("return \"Datadog.testlogger.dll\";", "return Path.Combine(Path.GetDirectoryName(FilePath) ?? string.Empty, \"Datadog.testlogger.dll\");");
    ...
    private static string FixKnownNullabilityIssues(string filePath, string content)
    {
        switch (Path.GetFileName(filePath))
        {
            case "CoverageCollector.cs":
                return content
                      .Replace("public override void Initialize(XmlElement configurationElement, DataCollectionEvents events, DataCollectionSink dataSink, DataCollectionLogger logger, DataCollectionEnvironmentContext environmentContext)",
                      ...
                      .Replace("_logger = new DataCollectorLogger(logger, environmentContext.SessionDataCollectionContext);",
                               "_logger = environmentContext is null ? null : new DataCollectorLogger(logger, environmentContext.SessionDataCollectionContext);")
                      .Replace("private bool Initialize(XmlElement configurationElement)",
                               "private bool Initialize(XmlElement? configurationElement)");
            case "DataCollectorLogger.cs":
                return content.Replace("textToLogger = exception?.ToString();", "textToLogger = exception.ToString();");
            case "HexString.cs":
                return content
                      .Replace("BitConverter.GetBytes(upper).CopyTo(bytes.Array, bytes.Offset);",
                      ...
                      .Replace("BitConverter.GetBytes(lower).CopyTo(bytes.Array, bytes.Offset + sizeof(ulong));",
                               "BitConverter.GetBytes(lower).CopyTo(buffer, bytes.Offset + sizeof(ulong));");
            case "XmlNodeConverter.cs":
                return content.Replace("d.XmlResolver = null;", "d.XmlResolver = null!;");
            default:
                return content;
        }
    }
    ...
    static string GenerateWarningDisablePragma(string filePath)
    {
        var value = "#pragma warning disable " +
        ...
        if (filePath.Contains("Datadog.Trace.Coverage.collector"))
        {
            value += "\n#nullable enable";
        }
