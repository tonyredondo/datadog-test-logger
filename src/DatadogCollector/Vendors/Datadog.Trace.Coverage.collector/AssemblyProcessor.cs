...
using DatadogTestLogger.Vendors.Datadog.Trace.Ci.Coverage.Attributes;
using DatadogTestLogger.Vendors.Datadog.Trace.Ci.Coverage.Metadata;
using DatadogCollector.Vendors.Mono.Cecil;
using DatadogCollector.Vendors.Mono.Cecil.Cil;
using CallSite = DatadogCollector.Vendors.Mono.Cecil.CallSite;
using MethodAttributes = DatadogCollector.Vendors.Mono.Cecil.MethodAttributes;
using TypeAttributes = DatadogCollector.Vendors.Mono.Cecil.TypeAttributes;
...
namespace DatadogTestLogger.Vendors.Datadog.Trace.Coverage.Collector
{
    internal class AssemblyProcessor
    {
                ...
                for (var typeIndex = 0; typeIndex < moduleTypes.Count; typeIndex++)
                {
                    ...
                    for (var methodIndex = 0; methodIndex < moduleTypeMethods.Count; methodIndex++)
                    {
                        ...
                        if (moduleTypeMethod.HasBody)
                        {
                             ...
                             *
                             *  internal static class MyMathClass
                             *  {
        ...
        private string GetDatadogTracer(TracerTarget tracerTarget)
        {
            ...
            return Path.Combine(_settings.TracerHome, targetFolder, "Datadog.testlogger.dll");
        }
        ...
        private string CopyRequiredAssemblies(AssemblyDefinition assemblyDefinition, TracerTarget tracerTarget)
        {
            try
            {
                ...
                var datadogTraceDllPath = Path.Combine(_settings.TracerHome, targetFolder, "Datadog.testlogger.dll");
                var datadogTracePdbPath = Path.Combine(_settings.TracerHome, targetFolder, "Datadog.testlogger.pdb");
